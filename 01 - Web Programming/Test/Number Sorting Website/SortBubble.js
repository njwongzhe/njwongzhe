 ...
 */
let bubbleSort = async function (sortMode, arrNum, processBool) {
    let sorted = false;
    ...
    if(processBool === true) {
        for(let pass = 1; (pass < arrNum.length); pass++) {
            ...
            for(let x = 0; x < (arrNum.length - pass); x++) {
                if(sortJudge(sortMode, Number(arrNum[x].innerText), Number(arrNum[x + 1].innerText))) {
        ...
        for(let pass = 1; (pass < arrNum.length); pass++) {
            sorted = true;
            for(let x = 0; x < (arrNum.length - pass); x++) {
                if(sortJudge(sortMode, arrNum[x], arrNum[x + 1])) {
